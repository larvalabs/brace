...
- **URLs** — `Url.to("/users/{id}", 42)` builds a path from a route pattern. Reverse routing: name a route at registration (`app.get("/users/{id}", ctrl::show).name(Routes.USER)`) and build links with `Url.to(Routes.USER, 42)` — works in templates too, so the pattern is written once. Keep names as constants in a `Routes` class.
