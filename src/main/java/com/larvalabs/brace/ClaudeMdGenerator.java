...
- **URLs** — `Url.to("/users/{id}", 42)` builds a path from a route pattern. Reverse routing: name a route at registration (`app.get("/users/{id}", ctrl::show).name(Routes.USER)`) and build links with `Url.to(Routes.USER, 42)` — works in templates too, so the pattern is written once. Keep names as constants in a `Routes` class. Query strings: pass a record as the last argument (`Url.to(Routes.LIST, new ListQuery(q, page))`) and read it in the handler with `req.form(ListQuery.class)`.
