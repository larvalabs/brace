...
- **Database** — Hibernate StatelessSession wrapper. `db.find()`, `.findOr404()`, `.insert()`, `.update()`, `.delete()`, `.query()`, `.queryOne()`, `.queryOneOr404()`, `.findBy()`, `.count()`, `.sql()`. Per-request transactions. HQL with `?` positional params. Paged lists: `db.paginate(Post.class, "... ORDER BY ...", req, 20)` returns `Paged<T>` with `.items()`, totals and `.links()`/`.prevUrl()`/`.nextUrl()` that keep the current filters, `.map(fn)` to convert entities to views/DTOs; `Paged.slice(list, req, n)` for in-memory lists.
