...
**After (0.1.9+) — records read by the handler and written by links:**

```java
public record ListQuery(String collector) {}
public record Page(Integer page) {          // one record, shared by every paginated route
    public static Page of(int n) { return new Page(n <= 1 ? null : n); }
...
// links, redirects, templates — trailing records become the query string
Url.to(Routes.LIST, new ListQuery(addr), Page.of(2))  // "/catalog/list?collector=0xabc&page=2"
Url.to(Routes.USER_POSTS, user.id, Page.of(2))         // "/users/42/posts?page=2"
```

```html
@import app.queries.ListQuery
@import app.queries.Page
<a href="${Url.to(Routes.LIST, new ListQuery(addr), Page.of(2))}">Page 2</a>
```

Details:

- Records at the **end** of the arguments are the query; several are written in argument
  order. A record followed by a path argument throws, and so do two records declaring the
  same component name. A typed-ID record (`record UserId(long value)`) is not a path
  value; pass its field (`id.value()`).
...
- Keep query records plain and give each concern its own record, with pagination as one
  shared `Page` record (above) rather than a `page` component and `withPage` method on
  every filter record. Normalize incoming values (case, blank strings) in the handler. See
