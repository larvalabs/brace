...
All round-two findings are fixed except R10, a `requireSession` startup-throw decision
that was settled and applied on `main` after merge. This file is the per-finding tracker
for the second code-review round; round one and the full writeup are in the review record,
