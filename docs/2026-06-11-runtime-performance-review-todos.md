...
Of 52 findings (9 High, 21 Medium, 22 Low): all 9 Highs and 19 of 21 Mediums are fixed;
12 of 22 Lows are fixed, 4 documented, and 1 won't-fix; M11, M21, and L7/L8/L9/L11/L14 are
deferred to 0.1.8 with rationale in this file. The narrative writeup, the macro and JMH
benchmarks, and the merge-gate code review are in the review record,
[`docs/reviews/2026-06-runtime-performance-fable-5.md`](reviews/2026-06-runtime-performance-fable-5.md).
...
- [~] **M11: Hibernate/Hikari autocommit dance per transaction**, `DatabaseFactory.java:160-181`: `hibernate.hikari.autoCommit` left true, `hibernate.connection.provider_disables_autocommit` unset. **Caveat:** the read-only handler path (`BraceHandler.java:341`) runs queries with no transaction, relying on autocommit; flipping this requires giving the read-only path an explicit lifecycle first. Model: frontier (correctness coupling). **DEFERRED to 0.1.8 (Matt, 2026-06-13).** Rationale: this is a genuine trade-off, not a free win. (1) On pgjdbc the per-transaction `setAutoCommit(false/true)` toggle is mostly *local flag flips*, not wire round-trips, so the write-path saving is modest. (2) The read-only path skipping begin/commit is a *deliberate* optimization; it saves a BEGIN/COMMIT round-trip pair per read request; flipping autocommit forces an explicit (read-only) transaction onto that path, *adding* a COMMIT round-trip per read and potentially net-regressing read-heavy workloads. Net effect is unclear without a real-Postgres measurement (H2 is in-process → round-trips ~free → would mislead). Correctness-coupled + uncertain net win → not worth shipping in the 0.1.7 perf release. **When picked up in 0.1.8:** first measure with `benchmark/.../Profile.java` against the `*IT` Postgres tier to quantify toggle-cost vs. added read-path commit, then decide; if taken, give read-only handlers an explicit read-only transaction lifecycle before flipping `provider_disables_autocommit=true` + Hikari `autoCommit=false`.
...
- [x] **M15: Cache `getOrSet` runs the user supplier inside `ConcurrentHashMap.compute`**, `InMemoryBackend.java:62-72` (verified: `store.compute` at `:67` invokes the supplier), via `Cache.java:105-110`. Suppliers are typically DB queries: the CHM bin lock is `synchronized`, so the supplier's blocking I/O pins a carrier thread on JDK 21–24, and unrelated keys in the same bin stall behind it. Keep single-flight but move work outside the map op (per-key in-flight `CompletableFuture` via `putIfAbsent`, or per-key `ReentrantLock`). Model: frontier-lite (single-flight semantics under exceptions). **Resolved as proposed (per-key `CompletableFuture`):** `getOrCompute` now `putIfAbsent`s a placeholder future into a new `inFlight` map; the winner runs the supplier **outside any map operation** then `store.put`s the result, losers `join` the future. No `ConcurrentHashMap` bin lock is held across the (blocking) supplier, so it neither pins a vthread carrier (JDK 21–24, JEP 491) nor stalls unrelated keys hashing to the same bin; and single-flight is now per *exact key* rather than per bin (strictly better). Single-flight preserved: a late second leader (winning `putIfAbsent` after the first finished and cleared its slot) re-checks the store and returns the stored value without re-running the supplier; the only re-run is when the entry genuinely vanished (expiry/delete). **Failure semantics (the frontier-lite bit):** supplier throw → `completeExceptionally` wakes awaiters with the *unwrapped* original exception (an `await` helper strips `CompletionException`), nothing is cached, the in-flight slot is cleared, every current caller retries on its next call; the non-concurrent case is byte-identical to before. The `Computed.hit` contract (false only when the supplier ran) is unchanged, so the facade's hit/miss counters are unaffected. 3 new `CacheTest` cases: single-flight under 16-thread contention (supplier runs exactly once), throw-doesn't-cache-and-retries, and 8-thread concurrent-throw-sees-unwrapped-exception. Suite 833/833.
...
- [x] **M17: Shared-backend rate limiter adds a full extra DB transaction per request through it**, `RateLimiter.java:199-209` → `Counters.java:43-65` (session + tx + upsert + commit per check, on top of the request's own transaction). Intentional design (B4, documented), but a per-instance micro-batch (count locally, flush delta every ~250ms or K hits) keeps fleet accuracy within a flush interval and cuts DB ops 10–100x on hot endpoints. Model: frontier (accuracy/consistency trade-off is a documented posture change). **Resolved (Matt-directed design, 2026-06-13) as negative cache + batched best-effort counting + two knobs:** (1) **Negative cache**, once a counter key is over the limit for the current window, the instance rejects it from a local `blockedUntil` map with **zero DB calls** until the window rolls. Sound because fixed-window counts are monotonic within a slot (an over-limit key stays over), so the cached 429 == the counter's decision; and it's exactly the abusive-client load we most want to shed. (2) **Batched flush**, each instance buffers increments and flushes them in **one** multi-row `Counters.incrementBatch` (new: Postgres `INSERT … ON CONFLICT … RETURNING` multi-VALUES; H2 portable loop in one tx) every `maxRequests / divisor` requests. The flush is a combined read+write: `RETURNING` refreshes `lastKnownGlobal`, so a machine learns the fleet count only at flush time (every N reqs). Per-request decision = `lastKnownGlobal + localPending`. Because the threshold counts *total* buffered requests (not keys), a unique-key flood collapses to `R/N` writes too; **DB writes bounded by request rate, not 1:1, regardless of key cardinality** (Matt's key insight; no auto-flush needed; below threshold no single-instance breach is possible). Flush failure **retains** the buffered counts (restored to the buffer) + arms a 1s retry cooldown → keeps enforcing per-instance during a DB blip, never 500s. (3) **`Brace.rateLimitBatchDivisor(int)`** (default `RateLimiter.DEFAULT_BATCH_DIVISOR = 10`); accuracy↔DB-load knob, read live (applies to limiters built before `start()`). (4) **`Brace.sharedRateLimiting(false)`**, opt out of all rate-limiter DB traffic (pure per-instance; effective limit ≈ K×N). **Accuracy posture (Matt accepted):** with no sticky routing a burst across K instances can overshoot by ~`maxRequests/divisor × K` before enforcement engages (worst case divisor=1 ⇒ K×N, but batched *recovers*; blocks rest of window; vs per-instance which stays loose every window); single-instance/single-key abuse always caught immediately (own count, no lag). **divisor=10 keeps small limits near-exact** (e.g. `perIp(5)` → flushThreshold 1 → flush every request), so the exact-guarantee `RateLimiterPostgresIT` (limits 5 & 2) passes UNCHANGED; only large limits batch. Suite 840/840 H2 + CountersPostgresIT 5/5 + RateLimiterPostgresIT 3/3 on real PG. Docs: BRACE-AGENTS.md Rate Limiting + Scaling, migration guide, `docs/2026-06-07-rate-limiter-load.md` (M17 section).
- [x] **M18: WebSocket broadcast has no slow-consumer backpressure**, `WsRegistry.java:51-58`, `WsContext.java:32-34` (`sendText(message, Callback.NOOP)`). A stalled client makes Jetty queue outgoing frames unboundedly (broadcast rate × message size per slow client). Track per-session pending sends via the callback and drop/close past a threshold; also consider Jetty's `setMaxOutgoingFrames`. Model: frontier-lite. **Resolved as:** per-`WsContext` `AtomicLong queuedBytes` + `AtomicBoolean closed`. `send` replaces `Callback.NOOP` with `Callback.from(succeed, fail)` that adds the message size before the send and subtracts it when Jetty reports the frame flushed; before sending, if `queuedBytes + size` exceeds the cap the connection is **force-closed once** with `TRY_AGAIN_LATER` and the send skipped. A send failure (broken connection) likewise marks closed; Jetty's `onClose`/`onError` already call `leaveAllRooms`, so the close reuses the existing cleanup path. Per connection, so a slow client never blocks healthy room members. Cap is configurable via new `Brace.wsMaxQueuedBytes(long)` (default 4 MB), plumbed through `WsRegistry`; queued size is approximated by `message.length()` (a cheap proxy; a safety threshold, not exact accounting). **Note on the tracker's `setMaxOutgoingFrames` suggestion:** that method is NOT on Jetty 12.0.33's high-level `org.eclipse.jetty.websocket.api.Session` (only the lower-level `core` session), so the application-level counter is the portable mechanism and the Jetty knob was deliberately skipped rather than reaching into internals. New `WebSocketTest.slowConsumerIsForceClosedByBackpressure` drives a raw socket that completes the upgrade then never reads (the JDK `HttpClient` WebSocket auto-drains, so it can't simulate a stalled TCP consumer); asserts the synchronous `ws-slow-consumer-closed` LogTap event (the close handshake can't complete against a wedged socket, so the server-side `onClose` is timeout-delayed and not a usable signal). BRACE-AGENTS.md + migration-guide entries. Suite 834/834.
...
- [ ] **M21: Cold start is fully serial: framework Flyway → app Flyway → SessionFactory → ops seed → Jetty**, `DatabaseFactory.java:38-40`, `Brace.java:509-524,599`. `buildSessionFactory` has no data dependency on the migrations (`hbm2ddl.auto=none`, `DatabaseFactory.java:163`) yet waits for both Flyway runs (each with its own scanner + unpooled connections). Typical serialized cost: Flyway 200–600ms + SessionFactory 0.5–1.5s. Fix: build the SessionFactory concurrently with the migrations (join before the constructor returns); optionally lazy Hikari fill (`initializationFailTimeout`). Jetty-last ordering is correct; keep it. ~30–50% cold-start cut for DB apps. Model: frontier (failure-ordering semantics: migration failure must still prevent serving). **DEFERRED to 0.1.8 (Matt, 2026-06-13): cold-start latency is not a priority; instances are long-lived so the win amortizes to ~zero after boot, and overlapping the SessionFactory build with migrations adds a concurrency hazard (must still fail-closed on migration error + not leak a half-built SessionFactory/pool) to the one place in the framework that's currently dead-simple. Like M11, wants real-PG measurement (Profile.java vs the *IT tier) before committing; H2's ~free round-trips would mislead. The bigger chunk of that 0.5–1.5s is often Hikari pool fill, so `initializationFailTimeout` lazy-fill is the safer lever to weigh first.**

## Low

- [x] **L1: `Invoker.fromFunction` allocated per request for plain `Handler` routes**, `BraceHandler.java:238-243`; `Brace.java:298` and the `/ops/*` registrations pass null invokers. Build the invoker once at registration. Model: smaller. **Resolved:** moved the `Invoker.fromFunction(handler)` build into `Router.add(String, String, Handler)`; the single overload that every plain-`Handler` registration funnels through (`Brace.get/post/...(Handler)`, `RouteGroup`, and all `/ops/*` routes). Every other handler type already built its invoker at registration; only this path left the Route's invoker null, forcing `BraceHandler` to allocate a fresh `fromFunction` per request. `BraceHandler`'s null-invoker fallback is kept as a defensive guard (now effectively unreached from the registration API). 127/127 across Router/Route/Invoker/Ops/RouteGroup tests.
- [x] **L2: `Invoker.build` (the `Method.invoke` path) is dead code**, `Invoker.java:34-64` has zero callers in src/main (verified by grep): every `app.get(...)` overload wraps lambdas in direct-call anonymous Invokers at registration (`Invoker.java:71-132`), so **no reflection runs on the request path**, an earlier draft of this finding claimed otherwise and was wrong. Delete `Invoker.build` (or, if kept for a future controller-scanning feature, add `setAccessible(true)` at build time and a comment). Model: smaller. **Resolved (kept, not deleted):** `Invoker.build` is the documented entry point for a future controller-method-scanning feature and is exercised by `InvokerTest`, so it stays; but it's now hardened and documented per the finding's "if kept" branch: a Javadoc states it is the ONLY reflective invoker and is NOT on the request hot path (all `app.get(...)` routes use the direct-call `fromXxx` invokers), and `method.setAccessible(true)` is set once at build time so the reflective path also works for methods on non-public controller classes (previously a latent `IllegalAccessException`). `InvokerTest` 3/3.
...
- [x] **L4: `Request.ip()` compiles a regex per call; `TrustedProxies.contains` allocates `BigInteger`s per check**, `Request.java:282`, `TrustedProxies.java:121-130`. Hoist a static `Pattern` (or hand-rolled digit check); cache `network.getAddress()` in `CidrRange`. Model: smaller. **Resolved:** (1) `Request.stripPort` replaced `portPart.matches("\\d+")` (compiles a fresh `Pattern` per `ip()` resolution) with a hand-rolled `isAllDigits` char-scan. (2) `CidrRange` now precomputes the masked network bytes + a per-byte `maskBytes` at construction; `contains()` is an allocation-free byte-wise masked compare; no per-check `BigInteger`s, no repeated `network.getAddress()` clones (only the incoming `addr.getAddress()` clone remains, unavoidable as `InetAddress` always copies). `createMask` rewritten to emit `byte[]` directly (full-`0xFF` bytes + one partial byte for the remainder bits). Dropped the now-unused `java.math.BigInteger` import. Behavior identical: `RequestIpTest` 32/32, `TrustedProxiesTest` 16/16 (covers `/8`,`/12` partial-byte mask, `/16`, IPv6 `/32` + single `::1`), `RequestTest` 10/10.
...
- [x] **L6: `convertPositionalParams` re-scans the HQL string on every execution**, `Database.java:370-511`, called from every query method. Bounded `ConcurrentHashMap<String,String>` memo (call sites are code literals → naturally small cardinality). Model: smaller. **Resolved:** split into a cached entry point (`convertPositionalParams`, still package-private for the direct unit tests) wrapping the pure scanner (now `static scanPositionalParams`). Memo is a static `ConcurrentHashMap` bounded at `PARAM_CACHE_MAX = 1000`: a hit returns the rewritten string with no scan; on a miss it scans, and only `putIfAbsent`s while under the cap so a caller building dynamic query strings (e.g. `queryIn`'s per-IN-size HQL, L7) can't grow it unbounded; it just falls back to a fresh scan. Scanner made `static` (it already used no instance state). `DatabaseTest` 42/42 (includes the conversion unit tests), `DatabaseIntegrationTest` 6/6.
- [~] **L7: `queryIn` generates one HQL string per IN-list size**, `Database.java:94-104`. Pad the list to the next power of two (repeat last value) to bound plan-cache/prepared-statement churn. Model: smaller. **PUNTED to 0.1.8 (Matt, 2026-06-13).**
- [~] **L8: `Session` JSON parser boxes scan indices as `String`**, `Session.java:344,353,369,386` (`String.valueOf(i)` → `Integer.parseInt` per key/value per decrypt). Small index-holder record instead. Model: smaller. **PUNTED to 0.1.8 (Matt, 2026-06-13).**
- [~] **L9: `Jobs.parallel` retains one `Thread` handle per item regardless of concurrency cap**, `Jobs.java:97-113`. Join in waves or acquire-all-permits at the end; keeps it O(concurrency). Model: smaller. **PUNTED to 0.1.8 (Matt, 2026-06-13).**
...
- [~] **L11: Page cache thundering herd on expiry; redundant cross-instance Postgres sweeps**, `Cache.java:317-321` (get-then-set, no single-flight for pages), `PostgresBackend.java:182-189` (global DELETE every 30s from every instance). Model: smaller. **DEFERRED to 0.1.8 (Matt, 2026-06-13); two separable items; the valuable half is bigger than a Low and gets the stale-while-revalidate (SWR) treatment.**

  **L11(A); page-cache thundering herd.** `CachedHandler.apply` (`Cache.java:341-355`) does a manual get-then-set, so on a hot page's expiry every concurrent request runs the full `handler.apply` render (N renders for one key). M15's single-flight does NOT cover this: (1) it lives in `InMemoryBackend.getOrCompute`, reachable only via `cache.getOrSet(...)`, which the page handler never calls; (2) `getOrSet` drops tags (`backend.setBytes(..., NO_TAGS)`, `Cache.java:125`; in-memory path passes none either) and doesn't single-flight the serializing backend; so routing pages through today's `getOrSet` would silently break `clearTag` invalidation. **0.1.8 design = stale-while-revalidate, not single-flight-and-wait** (Matt's call): on TTL expiry the first request to notice fires ONE async (vthread) regen and is served the still-present stale value along with the rest of the herd; the regen atomically swaps the entry in (`setInternal` with ttl + tags) on completion; nobody blocks, exactly one regen. **Correctness boundary that makes it safe:** SWR applies to TTL expiry ONLY; explicit invalidation (`clearTag`/`delete`) must hard-remove and bypass grace, so the event-driven "importantly incorrect" cases (publish/edit/permission change) never serve stale; the next request regenerates. **Caveat:** SWR covers expiry of an existing entry, NOT a cold miss (key never populated, or evicted by memory pressure / `clear()`); there's no stale value to serve, so the complete fix is BOTH single-flight-on-cold-miss AND SWR-on-expiry (full CDN behavior). Mechanics that make it a 0.1.8 item, not a Low: a "expired-but-usable" grace state must be plumbed through BOTH `InMemoryBackend` and `PostgresBackend` (today expiry returns null / deletes the row); async regen needs to capture the `Request` for off-thread use (safe for the no-DB GET page contract but a sharp edge; treat Request as immutable post-response); a per-key regen-in-flight guard (fire-and-forget, keep serving stale on regen failure + clear the guard to retry → better availability than a waiting single-flight); and it changes default freshness semantics → migration-guide entry. Wants real-PG measurement + a "16 threads → 1 render, herd served stale" test. Rejected the smaller per-instance single-flight-and-wait for 0.1.7 to avoid landing-then-reworking; one good change in 0.1.8 instead.

  **L11(B); redundant cross-instance Postgres sweeps.** `PostgresBackend.evictExpired()` (`:181-189`) is a global `DELETE … WHERE expires_at < now()` run by EVERY instance on its own 30s timer; expiry is already enforced on read, so it's pure space reclamation and N identical fleet-wide DELETEs are wasteful-but-harmless. Marginal value; cheapest safe mitigation is jitter/lengthen the interval. PUNTED (fold into the 0.1.8 cache work or drop).
...
- [x] **L13: `Storage` has no streaming path: whole object ≥2× in heap per upload**, `Storage.java:109` (`byte[]`), with `UploadedFile.bytes()` already buffered. Acceptable for stated scope; document the limitation, consider a temp-file + `BodyPublishers.ofFile` variant later. Model: n/a (docs) or frontier-lite (streaming variant). **Resolved (documented):** `docs/scaling.md` "Caveats & limits" now states `Storage.put` buffers the whole object as a `byte[]` (≥2× heap per upload), recommends `app.maxUploadSize(...)` + heap sizing for large media, and records the temp-file/`BodyPublishers.ofFile` streaming variant as a future option. No code change; acceptable for stated scope.
- [~] **L14: RateLimiters are never unregistered; one immortal cleanup vthread each; each runs the global sweep**, `RateLimiter.java:20,61,246-273`. Fine for `main()`-time construction; leaks if created dynamically or in tests. Add `close()`, centralize the sweep in one place. Model: smaller. **PUNTED to 0.1.8 (Matt, 2026-06-13).**
- [x] **L15: WebSocket rooms use `CopyOnWriteArraySet`: O(n) copy per join/leave**, `WsRegistry.java:32`. `ConcurrentHashMap.newKeySet()` keeps broadcast iteration safe with O(1) membership churn. Model: smaller. **Resolved:** `join` now creates `ConcurrentHashMap.newKeySet()` per room instead of `CopyOnWriteArraySet`; O(1) add/remove vs the O(n) array copy on every membership change (quadratic for high-churn rooms). Set identity stays stable so `leave`'s two-arg `rooms.remove(room, members)` empty-room reap is unaffected; the new weakly-consistent iterator is still safe for the lock-free `deliverLocal` fan-out (same just-joined/just-left tolerance the snapshot iterator had, and `ctx.send` already no-ops on a closed connection via M18). Dropped the now-unused `CopyOnWriteArraySet` import. `WebSocketTest` 7/7 + `WebSocketFanoutPostgresIT` 2/2.
- [x] **L16: Postgres message bus: session + transaction + `pg_notify` round trip per broadcast**, `PostgresMessageBus.java:77-96`, plus opportunistic DELETE on the spill path (`:121-124`). Fine at chat rates; batch/coalesce if per-tick fan-in becomes a use case. Model: note only. **Resolved (documented):** `docs/scaling.md` "Caveats & limits" records the per-broadcast session+tx+`pg_notify` cost, notes it's fine at human chat rates, and flags coalescing/batching as the next step for a high-fan-in firehose. Note-only finding; no code change.
- [x] **L17: `ErrorStore.list(status, since)` fetches all rows/columns then filters in Java**, `ErrorStore.java:198-209`; bounded at 1000 rows but each carries full stack traces + headers; the deploy post-check and `RegressionTracker.seed()` hit it. Push `first_seen >= ?` + column subset into SQL. Model: smaller. **Resolved (since-pushdown):** `list(status, since)` no longer calls `list(status)` and post-filters in Java; it builds one parameterized query that appends `AND first_seen >= ?` (bound as `Timestamp.from(since)`, matching how the insert path writes the column) so rows older than the cutoff; and their full `stack_trace`/`request_detail`/`request_headers` payloads; are never fetched. `list(status)` now delegates to `list(status, null)`. Semantics preserved exactly: a NULL `first_seen` is never `>= ?`, mirroring the old `firstSeen != null && !isBefore` Java drop; `status` maps to a fixed clause (no user SQL). **Column-subset deliberately NOT taken:** the repeated caller (`/ops/errors`) needs all columns for display, and the only over-fetcher is `RegressionTracker.seed()` (`list(null)`), a one-time startup read bounded at 1000 rows; not worth forking the query + touching RegressionTracker. Verified by `ErrorStoreTest.listSinceReturnsOnlyErrorsAfterTimestamp` (cutoff returns only the newer error); 135/135 across ErrorStore/Regression/Ops incl. Postgres ITs.
- [x] **L18: `Assets.url()` does two `stat` syscalls + path normalization per call even on hash-cache hits**, `Assets.java:54-84` (`Files.isRegularFile` + `Files.getLastModifiedTime`; per-mapping base path re-normalized per call). In prod mode, cache the final URL keyed by path with a short re-stat interval; hoist the base-path normalization to construction. (Content-hash caching by `(path, mtime)` verified present.) Model: smaller. **Resolved (scoped to the base-path half):** the constructor now precomputes each mapping's absolute-normalized base `Path` once (new `ResolvedMapping(prefix, base)` record); `resolve()` reuses it instead of redoing `Path.of(dir).toAbsolutePath().normalize()` per mapping on every `Assets.url()` call (which fires for each asset reference in each rendered page → template-render hot path). The finding's other idea; a wall-clock re-stat interval that skips the `getLastModifiedTime` stat on cache hits; was deliberately NOT taken: it conflicts with the documented `(path, mtime)` immediate-invalidation contract that `AssetsTest.hashChangesWhenFileContentChanges` enforces (two `url()` calls <1s apart with different mtimes must return different hashes). The two stats stay; only the redundant normalization is removed. `AssetsTest` 10/10, `StaticFilesTest` 17/17.
- [~] **L19: JobPoller + JobScheduler always start when a database exists, even with zero jobs**, `Brace.java:627-630`; `JobPoller.java:34-58` (~8,600 idle `scheduled_jobs` queries/day/instance), `JobScheduler.java:70-82` (scheduler pool with no jobs). Start the poller lazily on first `Jobs.schedule(...)`; skip the scheduler pool when nothing is registered. Model: smaller. **WON'T FIX (Matt, 2026-06-13): not worth the complexity/risk for the gain.** The poller half needs a static bridge from `Jobs.schedule` (a `static` method) to the instance `JobPoller`, or a durable job enqueued at runtime by a handler would never be polled. The scheduler half collides with the `register()`/`daily()` late-registration branches, which only schedule when `scheduler != null`; skipping the empty pool at `start()` would silently break a job registered after `start()` (the exact bug class the M14 `daily()` fix closed). The gain is small: a single no-op `SELECT … SKIP LOCKED` returning zero rows every 10s + one idle scheduler thread. Cost/benefit doesn't justify the added concurrency surface.
- [x] **L20: Hikari `minimumIdle == maximumPoolSize` (10), not separately configurable**, `DatabaseFactory.java:29,166-167`. Fixed-size pools are Hikari's recommended posture, but a few instances × 10 + the message-bus raw connection + JobPoller can exhaust small PaaS `max_connections`; each idle PG connection costs ~1–10MB server-side. Expose `minimumIdle` or document sizing in `docs/scaling.md`. Model: smaller (mostly docs). **Resolved (documented):** `docs/scaling.md` "Caveats & limits" now spells out the per-instance connection budget (`poolSize` default 10, fixed-size by design, + 1 raw `LISTEN` connection when WebSockets are used), the fleet total `N × poolSize + (N if WS)`, the ~1–10 MB/idle-connection server cost, the small-PaaS-ceiling hazard, and how to tune via the `DatabaseFactory(..., poolSize)` constructor; noting `minimumIdle` is intentionally not separately configurable. Docs path chosen over exposing `minimumIdle` (the finding's either/or).
- [x] **L21: Static files re-read from disk and fully buffered per request; no Cache-Control/ETag**, `BraceHandler.java:506` (`Files.readAllBytes` per request), no caching headers despite `Assets` fingerprinting existing for exactly this. Emit `Cache-Control: public, max-age=31536000, immutable` for `?v=` fingerprinted URLs (+ `Last-Modified`/`ETag` otherwise); optionally a small bounded cache keyed by (path, mtime). Model: smaller. **Resolved:** `serveStaticFile` now takes the `Request` (for query params + conditional headers) and emits, per file, a strong `ETag` (`"<size>-<mtime>"` hex) + `Last-Modified` (Jetty `DateGenerator.formatDate`). A `?v=` fingerprinted URL gets `Cache-Control: public, max-age=31536000, immutable`; any other URL gets `public, max-age=0, must-revalidate`. New `isNotModified()` honors `If-None-Match` (ETag wins per RFC 9110; handles `*`, comma lists, and `W/` weak prefixes) then `If-Modified-Since` (Jetty `DateParser.parseDate`, second-resolution compare) and short-circuits to a new `Result.notModified()` (304, empty body); so a client holding a current copy avoids the full `Files.readAllBytes`. The optional in-process byte cache was deliberately NOT added (keeps the change free of a memory-eviction policy; conditional GETs already remove the repeat-read cost). htmx (`/__brace/htmx.min.js`) is served from an in-memory field, not disk, so it's untouched. New `StaticFilesTest` cases: validators+revalidate Cache-Control, fingerprinted→immutable, `If-None-Match`→304, `If-Modified-Since`→304. Suite 17/17 in that file.
- [x] **L22: Jetty `QueuedThreadPool` left at defaults (max 200) alongside virtual threads**, `Brace.java:543-556`. Handlers run on virtual threads, so the platform pool only serves selectors/acceptors; cap it (e.g. 16) to make the footprint intentional. Cosmetic-to-small. Model: smaller. **Resolved (documented, not capped):** `docs/scaling.md` "Caveats & limits" now states handlers run on virtual threads so the `QueuedThreadPool` (max 200) only serves acceptors/selectors, that max is a ceiling not a reservation (small actual footprint), and it is intentionally left unconfigured. Chose to document the posture rather than hard-cap the max; a fixed low cap risks selector/acceptor starvation under high connection counts on large hosts, and the finding is explicitly cosmetic.
