...
**Phase 1 — Harness.** Extract a `PostgresTestBase` (singleton container, truncation-between-tests, a `DatabaseFactory` factory pointed at the container). ~~Wire the Maven tier (failsafe `*IT` or `-Ppg`).~~ ✅ done in Phase 0 (failsafe). ~~Add the PG tier to CI.~~ ✅ **done 2026-06-05** — `.github/workflows/ci.yml` runs `mvn verify` (surefire H2 + failsafe Postgres IT) on PRs and feature-branch pushes; `publish.yml`'s `mvn deploy` already traverses the verify phase, so main/tags run the IT before publishing. ubuntu-latest has Docker, so Testcontainers works with no extra runner setup.
