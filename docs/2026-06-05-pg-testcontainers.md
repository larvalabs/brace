...
**Phase 2 — Migrate the DB-sensitive tests.** Port the integration tests whose value depends on real DB semantics to also run on the PG tier. Leave the dialect-agnostic majority on H2.
  ...
  - ✅ **`ErrorStore` duplicate-row race done 2026-06-06** — `ErrorStorePostgresIT` (5 tests). This is the Tier 2a port and the fix in one move (they reinforce each other): the IT proves the property the check-then-insert can't hold under load — 50 virtual threads recording the same `(type, route)` fold into **exactly one row with an exact count** — and that property only holds because of the `ON CONFLICT` rewrite landed alongside it (see Phase 3). H2 `ErrorStoreTest` (14 tests) stays on the check-then-insert branch as the fast functional suite. Parity tests in the IT confirm the Postgres branch matches H2 semantics (fold-on-duplicate, new-row-on-resolved-recurrence, distinct-routes-separate).
  - ⬜ **Remaining ports:** `RegressionIntegrationTest`, the ops integration tests. Lower urgency — these are more CRUD/logic than concurrency.

**Phase 3 — Cash in the postgres-native simplifications.** With the tier proving the real path, land the Tier 2 items from the postgres-native doc.

  - ✅ **Tier 2a — `ErrorStore` `ON CONFLICT` + partial unique index, shipped 2026-06-06.** `ErrorStore.record` now branches on `DatabaseFactory.isPostgres()`: Postgres runs a single `INSERT … ON CONFLICT (error_type, route) WHERE resolved_at IS NULL DO UPDATE … RETURNING (xmax = 0)` (raw JDBC, so Hibernate doesn't misclassify the `RETURNING`; `xmax = 0` is the new-vs-repeat signal the regression listener needs), backed by a **partial** unique index so resolved errors stay out of the dedupe and a recurrence after resolve still gets a fresh row. H2 keeps check-then-insert. The index is a Postgres-only framework migration (`brace/db/migration_pg/V6`) added to Flyway's locations only on Postgres — a *separate top-level* dir, not a subdir of `brace/db/migration`, because Flyway scans recursively and the H2 tier would otherwise pick up DDL it can't parse. Guarded + proven by `ErrorStorePostgresIT`.
  - ✅ **Tier 1b — `TIMESTAMPTZ` + delete the hand-rolled parsers, shipped 2026-06-06.** Shared migration `V7__brace_timestamptz.sql` converts every framework instant column to `TIMESTAMP WITH TIME ZONE` (portable `SET DATA TYPE`, runs on both tiers); `ErrorStore.parseFirstSeen` and `RegressionTracker.parseInstant` deleted in favor of typed `Instant` reads. (Surfaced and fixed a latent test-tier bug: `PostgresSkipLockedClaimIT` hand-rolled Flyway with only the base location, so when a `DatabaseFactory`-based IT applied the pg-only `migration_pg/V6` into the shared history first, its `migrate()` failed validation — fixed by mirroring `DatabaseFactory`'s full location set.)
  - ✅ **Tier 1a — `SKIP LOCKED` batch claim, shipped 2026-06-06.** `JobPoller.pollAndExecute` branches on dialect: Postgres folds selection + claim into one `UPDATE … WHERE id IN (SELECT … ORDER BY run_at LIMIT 50 FOR UPDATE SKIP LOCKED) RETURNING …` transaction (disjoint batches, each row claimed exactly once, no per-row re-claim); H2 keeps the portable select-then-per-row-claim with the B7 row-count guard. `DurableJobConcurrencyPostgresIT` now exercises the batch path (80 jobs, 4 concurrent pollers, exactly-once).
  - ⬜ **Still to land:** `TEXT[]`+GIN for shared-cache tags, JSONB columns.
