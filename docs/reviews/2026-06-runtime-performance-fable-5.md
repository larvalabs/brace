...
| **M11** | **Autocommit dance — DEFERRED to 0.1.8** (see below) | — |
...
| **M21** | **Serial cold start — DEFERRED to 0.1.8** (see below) | — |
...
| L7 | Pad `queryIn` lists to a power of two — **PUNTED to 0.1.8** | — |
| L8 | `Session` JSON parser index boxing — **PUNTED to 0.1.8** | — |
| L9 | `Jobs.parallel` O(items) thread handles — **PUNTED to 0.1.8** | — |
| L10 | Memoize cache TTL parsing | `7510ba2` |
| L11 | Page-cache thundering herd + redundant PG sweeps — **DEFERRED to 0.1.8** (SWR design captured) | — |
...
| L14 | RateLimiters never unregistered / immortal cleanup vthread — **PUNTED to 0.1.8** | — |
...
| L19 | Lazy JobPoller/JobScheduler start — **WON'T FIX** (static-bridge + late-registration risk ≫ a no-op poll every 10s) | — |
...
- **L11 — page-cache thundering herd.** Bigger than a Low: M15's single-flight lives in
  ...
  **stale-while-revalidate** — on TTL expiry the first request fires one async regen and is
  served the still-present stale value with the rest of the herd; the regen atomically swaps in
  — **plus** single-flight on cold miss (SWR doesn't cover a never-populated key). Crucially,
...
| ID | Finding | Commit |
|---|---|---|
| CR#6 | Raw NUL bytes in `ErrorStore.java` (`'<NUL>'` char literal) made git/grep treat the file as binary — replaced with `' '` (identical value, zero behavior change) | `4d474a2` |
